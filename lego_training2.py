# With help from: https://www.kaggle.com/guillaumekarklins/cnn-classifier-96-accuracy-on-test-set
...
# We shuffle the list, but not at each iteration because as we don't want train,
# test and validation datasets to overlap on a new run
...
# Get the label corresponding to the image returning a number in the [0,50] range ,
# corresponding to the id in the earlier DataFrame
...
def prepare_for_training(ds, cache=False, shuffle_buffer_size=1000):
    ...
    ds = ds.repeat()
    ds = ds.batch(8)  # Batch by 8, this was the highest value possible with my GPU/CPU
    ds = ds.prefetch(buffer_size=1)  # Bufer size is an hyperparameter that hasn't been tweaked, set to 1.
